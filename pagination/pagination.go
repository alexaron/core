...
func Map() template.FuncMap {
	...
	f["PAGINATION"] = func(info Info, m map[string]interface{}) template.HTML {
		...
		if strings.HasPrefix(fmt.Sprintf("%v",currentURI), "/account") {
			currentURI = strings.TrimPrefix(fmt.Sprintf("%v",currentURI), "/account")
		}
		...
		if info.TotalPages <= 1 {
			pagediv = `<div class="pv10"></div>`
			showdiv = ""
		}

		return template.HTML(pagediv + showdiv)
	}
